...
const loadCategory = async () => {
    ...
    data.data.forEach(category => {
        ...
        <button onclick="handleCategory('${category.category_id}')" class="btn focus:bg-[#FF1F3D] active:bg-red-300 focus:text-white hover:bg-[#FF1F3D] hover:text-white bg-[#46454546] text-[#252525d3] normal-case btn-sm font-normal" id="${category.category_id}">${category.category}</button>
        `;
    });
}
...
// initially "all" button clicked and all category is shown
handleCategory('1000');
loadCategory()
